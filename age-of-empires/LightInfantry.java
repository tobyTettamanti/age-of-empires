...
public class LightInfantry extends Unit implements Attacking{

    public LightInfantry(double xLoc, double yLoc, Player player) {
        super(4, 40, 10, 2, xLoc, yLoc, player);
    }
    public LightInfantry (LightInfantry lightInfantry) {
        super(lightInfantry.getMorale(), 4, 40, 10, 2, lightInfantry.getXLoc(), lightInfantry.getYLoc(), lightInfantry.getPlayer());
    }
    ...
    public void attack(Tile initialAttackingUnitTile, Tile initialDefendingUnitTile, Pane mapPane) { // attack damage based on terrain, units morale, enemy morale
        if (initialDefendingUnitTile.getControllable().canAttack()) { // Controllable implements Attacking
            ...
            damageVal *= this.additionalFactors(initialDefendingUnitTile);
    ...
    public ImageView setImage(double xLoc, double yLoc) {
        try {
            ImageView imageView = new ImageView(new Image(new FileInputStream("Light_Infantry.png")));
            imageView.setX(xLoc);
            imageView.setY(yLoc);
            return imageView;
